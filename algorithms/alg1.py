...
def main():
    ...
    sorted_coeffs, coeffs = find_similar_text(data, 174430, 'category')
...
def dice_coeff(data, x_game_id, y_game_id, col_name):
    x = data[data['game_id'] == x_game_id]
    y = data[data['game_id'] == y_game_id]
    return utils.dice_coeff(x[col_name], y[col_name])
